...
# The design created by this script references the module:
#   audio_engine
# It builds ONLY the block design - the project and the RTL sources must already
# exist. Use scripts/build_project.tcl, which creates the project, adds rtl/*.v
# and the constraints, then sources this file. (The original write_bd_tcl
# project-creation boilerplate was removed so this does not create a stray
# project_1/myproj that collides with the real one.)

if { [get_projects -quiet] eq "" } {
   catch {common::send_gid_msg -ssname BD::TCL -id 2000 -severity "ERROR" \
      "No open project. Create or open a project first (e.g. run scripts/build_project.tcl), then source bd/soc_bd.tcl."}
   return 1
}
...
proc create_root_design { parentCell } {
  ...
  set au_din [ create_bd_port -dir O au_din ]
  set au_dout [ create_bd_port -dir I au_dout ]
  set au_bclk [ create_bd_port -dir O au_bclk ]
  set au_wclk [ create_bd_port -dir O au_wclk ]
...
\
  ...
  # Create instance: audio_engine_0, and set properties
  set block_name audio_engine
  set block_cell_name audio_engine_0
  if { [catch {set audio_engine_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $audio_engine_0 eq "" } {
  ...
  connect_bd_intf_net -intf_net axi_smc_M01_AXI [get_bd_intf_pins axi_smc/M01_AXI] [get_bd_intf_pins audio_engine_0/S_AXI]
  ...
  connect_bd_net -net au_dout_1  [get_bd_ports au_dout] \
  [get_bd_pins audio_engine_0/sdata_adc]
  connect_bd_net -net audio_engine_0_lrclk  [get_bd_pins audio_engine_0/lrclk] \
  [get_bd_ports au_wclk]
  connect_bd_net -net audio_engine_0_sclk  [get_bd_pins audio_engine_0/sclk] \
  [get_bd_ports au_bclk]
  connect_bd_net -net clk_wiz_0_clk_out1  [get_bd_pins processing_system7_0/FCLK_CLK1] \
  [get_bd_ports aud_mclk]
  connect_bd_net -net iis_ser_0_SDATA  [get_bd_pins audio_engine_0/sdata_dac] \
  [get_bd_ports au_din]
  ...
  [get_bd_pins audio_engine_0/clk_100] \
  [get_bd_pins audio_engine_0/S_AXI_ACLK]
  ...
  [get_bd_pins audio_engine_0/S_AXI_ARESETN]

  # Create address segments
  assign_bd_address -offset 0x40000000 -range 0x00001000 -target_address_space [get_bd_addr_spaces processing_system7_0/Data] [get_bd_addr_segs audio_engine_0/S_AXI/reg0] -force
